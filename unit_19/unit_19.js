...
function t1() {
    ...
    document.querySelector('div.out-1').textContent = text;
    return text;
}
...
function t2() {
    ...
    document.querySelector('div.out-2').textContent = result;
...
function t3() {
    w3 += 5;
    document.querySelector('div.out-3').textContent = w3;
}
...
function t4() {
    ...
    document.querySelector('div.out-4').textContent = text;
    return text;
}
...
/// ..... при котором на документе отлючается клик....
/// ... и отключает .....  --- КОГДА ВКЛЮЧАЕТ, А КОГДА ОТКЛЮЧАЕТ!???
function t8() {
    if (this.checked) {
...
function t16() {
    let w = window.getComputedStyle(this).width.replace(/\D/g, ''); // event.width.replace(/\D/g, '')
...
function t18() {
    console.log(event);
    this.textContent = window.getComputedStyle(this).width;
}
// ваше событие здесь!!!
...
function t19() {
    this.textContent = event.fromElement.className;
}
// ваше событие здесь!!!
